...
def parse_args():
    parser = argparse.ArgumentParser(description="Encode nuScenes images with any HF model")
    ...
    parser.add_argument("--output_path", type=str, default="./camera_image_embeddings_clip.pth")
...
model = CLIPModel.from_pretrained(args.model_name).eval().cuda()
...
for sample_token in tqdm(val_sample_tokens, desc="Encoding images"):
    ...
    for cam_name in CAMERAS:
        ...
        img_path = os.path.join(args.dataroot, cam_sample_data["filename"])
        try:
            ...
            cam_data[cam_name] = torch.zeros(512)

    camera_image_embs[sample_token] = cam_data

torch.save(camera_image_embs, args.output_path)
print(f"Saved image embeddings to {args.output_path}")
