...
def main():
        ...
        with torch.no_grad():
            for images, texts, all_points, offsets, sample_tokens, cameras in tqdm(val_loader, desc="Validation", leave=False):
                ...
                for i in range(B):
                    ...
                    point_dict = {"coord": coord.cpu().numpy(), "color": color.cpu().numpy(), "normal": normal.cpu().numpy()}
                    ...
                    feat = out['feat'].mean(dim=0) if isinstance(out, dict) else out.mean(dim=0)
    ...
    for epoch in range(args.epochs):
        ...
        for images, texts, all_points, offsets, sample_tokens, cameras in progress_bar:
            ...
            loss_fused_text = (F.cross_entropy(logits_fused_text, labels) + F.cross_entropy(logits_fused_text.T, labels)) / 2
